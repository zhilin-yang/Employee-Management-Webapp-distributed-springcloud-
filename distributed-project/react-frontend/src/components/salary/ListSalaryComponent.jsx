    ...
    render() {
        return (
            <div>
                ...
                <div className="row">
                    {this.state.role == 0 ?
                        <button className="btn btn-primary" onClick={this.addSalary}> Add Salary</button> : null}
                            ...
                            this.state.salaries.map(
                                salary =>
                                    <tr key={salary.id}>
                                        ...
                                        <td>
                                            {this.state.role == 0 ?
                                                <button onClick={() => this.editSalary(salary.employeeId)}
                                                        className="btn btn-info">Update </button> : null}
                                            {this.state.role == 0 ? <button style={{marginLeft: "10px"}}
                                                                            onClick={() => this.deleteSalary(salary.employeeId)}
                                                                            className="btn btn-danger">Delete </button> : null}


                                            <button style={{marginLeft: "10px"}}
                                                    onClick={() => this.viewSalary(salary.employeeId)}
